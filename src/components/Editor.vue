...
export default {
  ...
  computed: {
    ...
    ...mapGetters('data', ['computedSettings']),
  },
  mounted() {
    ...
    const classToggler = (toggle) => (discussionId) => {
      editorElt
        .getElementsByClassName(`discussion-editor-highlighting--${discussionId}`)
        // eslint-disable-next-line arrow-parens
    ...
    this.$watch(
      ...
      (discussionId, oldDiscussionId) => {
        if (oldDiscussionId) {
          editorElt
            .querySelectorAll(`.discussion-editor-highlighting--${oldDiscussionId}`)
            // eslint-disable-next-line arrow-parens
            .cl_each((elt) => elt.classList.remove('discussion-editor-highlighting--selected'));
        }
        if (discussionId) {
          editorElt
            .querySelectorAll(`.discussion-editor-highlighting--${discussionId}`)
            // eslint-disable-next-line arrow-parens
