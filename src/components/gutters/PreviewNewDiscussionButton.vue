  ...
  methods: {
    ...
    checkSelection() {
      ...
      this.timeout = setTimeout(() => {
        ...
        if (!store.state.content.revisionContent && editorSvc.previewSelectionRange) {
        ...
        this.coordinates = offset ? editorSvc.getPreviewOffsetCoordinates(offset) : null;
      }, 25);
    },
  },
  mounted() {
    this.$nextTick(() => {
      ...
      this.$watch(() => store.getters['layout/styles'].previewWidth, () => this.checkSelection());
      this.checkSelection();
    });
  },
};
