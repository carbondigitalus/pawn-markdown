...
export default {
   ...
   */
  async getTree({ token, owner, repo, branch }) {
   ...
   */
  async getCommits({ token, owner, repo, sha, path }) {
   ...
   */
  async uploadFile({ token, owner, repo, branch, path, content, sha }) {
    return repoRequest(token, owner, repo, {
      method: 'PUT',
   ...
   */
  async removeFile({ token, owner, repo, branch, path, sha }) {
    return repoRequest(token, owner, repo, {
      method: 'DELETE',
      url: `contents/${encodeURIComponent(path)}`,
      body: {
   ...
   */
  async downloadFile({ token, owner, repo, branch, path }) {
   ...
   */
  async uploadGist({ token, description, filename, content, isPublic, gistId }) {
   ...
   */
  async downloadGist({ token, gistId, filename }) {
   ...
   */
  async getGistCommits({ token, gistId }) {
   ...
   */
  async downloadGistRevision({ token, gistId, filename, sha }) {
    const result = (await request(token, {
