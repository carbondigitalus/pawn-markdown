...
const httpHeaderSafeJson = (args) =>
  args &&
  JSON.stringify(args).replace(/[\u007f-\uffff]/g, (c) => `\\u${`000${c.charCodeAt(0).toString(16)}`.slice(-4)}`);

const request = ({ accessToken }, options, args) =>
  networkSvc.request({
    ...options,
    headers: {
      ...(options.headers || {}),
      'Content-Type':
        options.body &&
        (typeof options.body === 'string' ? 'application/octet-stream' : 'application/json; charset=utf-8'),
...
export default {
   ...
   */
  async uploadFile({ token, path, content, fileId }) {
   ...
   */
  async downloadFile({ token, path, fileId }) {
   ...
   */
  async listRevisions({ token, path, fileId }) {
